module.exports.run = async (bot, message, args) => {
    ...
    try {
        ...
        if(teamData.players.includes(player[0])){
            message.channel.send("Le joueur `" + message.content + "` est déjà dans l'équipe").then((msg) => {
                setTimeout(() => {
                    if (!msg.deleted) msg.delete().catch();
                }, 10000)
            });
        ...
        await bot.specialChannel.createGame.get("addPlayerInTeam").run(bot, player[0], teamData.id, false);

        bot.specialChannel.startMenu.get("menu").run(bot, teamData.id);


        message.channel.send("Le joueur `" + message.content + "` a été rajouté à l'équipe").then((msg) => {
            setTimeout(() => {
                if (!msg.deleted) msg.delete().catch();
            }, 60000)
        });
